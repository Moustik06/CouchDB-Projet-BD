...
public class VehiculeDAO {
     ...
     * CouchDbConnector db = new StdCouchDbConnector("vehicule", new
     * StdCouchDbInstance(new
     * StdHttpClient.Builder().url("http://admin:admin@localhost:5984").build()));
     * db.create("marque", "marque", "json");
     * db.create("modele", "modele", "json");
     * db.create("plaque_imat", "plaque_imat", "json");
     * }
     * public ArrayList<JSONObject> findByCriteria(String marque, String modele,
     * String plaque_imat) {
     * CouchDbConnector db = new StdCouchDbConnector("vehicule", new
     * StdCouchDbInstance(new
     * StdHttpClient.Builder().url("http://admin:admin@localhost:5984").build()));
     ...
     * StdHttpClient.Builder().url("http://admin:admin@localhost:5984").build()));
     * ViewQuery query = new ViewQuery().allDocs().includeDocs(true);
     * query.designDocId("_design/vehicule").viewName("vehiculesParAgence").key(
     * idAgence);
     * List<CouchDbDocument> docs = db.queryView(query, CouchDbDocument.class);
     * ArrayList<JSONObject> jsons = new ArrayList<>();
     * for (CouchDbDocument doc : docs) {
     ...
     * StdHttpClient.Builder().url("http://admin:admin@localhost:5984").build()));
     * ViewQuery query = new ViewQuery().allDocs().includeDocs(true);
     * query.designDocId("_design/vehicule").viewName(
     * "vehiculesParMarqueAvecDetails").key(marque);
     * List<CouchDbDocument> docs = db.queryView(query, CouchDbDocument.class);
     * ArrayList<JSONObject> jsons = new ArrayList<>();
     * for (CouchDbDocument doc : docs) {
     * jsons.add(doc.getCurrentRevisionAsJson());
     ...
     * ViewQuery query = new ViewQuery().allDocs().includeDocs(true);
     * query.designDocId("_design/vehicule").viewName(
     * "vehiculesParPlaqueAvecDetails").key(plaqueImat);
     * List<CouchDbDocument> docs = db.queryView(query, CouchDbDocument.class);
     * ArrayList<JSONObject> jsons = new ArrayList<>();
     * for (CouchDbDocument doc : docs) {
     * jsons.add(doc.getCurrentRevisionAsJson());
